async function MELONsquad() {
  ...
  if (window.location.href.includes("%23")) {
    ...
    window.location.href =
      window.location.href.substring(0, hash) +
      "#" +
      window.location.href.substring(hash + 3);
  }
...
async function navBar() {
  ...
  //console.log("list");
  const response = await fetch("JSON/navbar.json").then((response) =>
    response.json()
  );
  json = response;
  //console.log(json);
...
function About() {
  section.innerHTML = '<h1 id="sectionHeading">About - Coming now!</h1>';
}
...
async function Events() {
  ...
  const response = await fetch("JSON/events.json").then((response) =>
    response.json()
  );
  events = response;
  console.log(events);
...
async function ChangeEvent() {
  ...
  if (pos1 == pos2) {
    ...
    window.location.href =
      window.location.href.substring(0, pos2) +
      "#" +
      events[
        document.getElementById("eventSelector").value
      ].name.toLowerCase()[0];
  ...
  eventSection.innerHTML = `
        ...
        <p>${
          events[document.getElementById("eventSelector").value].description
        }</p>
    `;
  if (events[document.getElementById("eventSelector").value].src == "local") {
    ...
    eventSection.innerHTML += `
            <audio id="eventMusic" autoplay onplay="APause()" onended="APlay()">
                <source src="Assets/Music/${
                  events[document.getElementById("eventSelector").value].sounds
                }" type="audio/mpeg">
            </audio>
        `;
  } else {
    ...
    <iframe width=\"560\" height=\"315\" src=\"https://www.youtube.com/embed/${
      events[document.getElementById("eventSelector").value].sounds
    }?controls=0&autoplay=1\" title=\"YouTube video player\" frameborder=\"0\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen onplay="APause()" onended="APlay()"></iframe>`;
  }
  ...
  for (image in events[document.getElementById("eventSelector").value].images) {
    ...
    eventSection.innerHTML += `
        ...
        <div id="eventImage">
                <a target="_blank" href="Assets/Graphics/${
                  events[document.getElementById("eventSelector").value].name
                }/${
      events[document.getElementById("eventSelector").value].images[image]
    }.png">    
                
                    <img src="Assets/Graphics/${
                      events[document.getElementById("eventSelector").value]
                        .name
                    }/${
      events[document.getElementById("eventSelector").value].images[image]
    }.png">
...
async function Recipes() {
  ...
  const response = await fetch("JSON/recipes.json").then((response) =>
    response.json()
  );
  json = response;
...
async function innerSchedule() {
  const response = await fetch("JSON/schedule.json").then((response) =>
    response.json()
  );
  json = response;
...
async function News() {
  console.log("news");
  section.innerHTML =
    '<h1 id="sectionHeading">MELONsquad & BLAHAJgang in the News - Coming soon...</h1>';
}

async function AddImageBoxes(imageDirectory, jsonFile, link = false) {
  const response = await fetch("JSON/" + jsonFile + ".json").then((response) =>
    response.json()
  );
  json = response;
  team = "";
...
async function HackathonHistory() {
  const response = await fetch("JSON/wins.json").then((response) =>
    response.json()
  );
  json = response;
  ...
  for (hacker in json) {
    ...
    for (hackathon in json[hacker].hackathons) {
      historystring += `<a href="${json[hacker].hackathons[hackathon].link}" target="_blank" id="noformat">`;
      historystring += AddImageBox(
        "Hackathons/" + json[hacker].hackathons[hackathon].name + ".png",
        json[hacker].hackathons[hackathon].name,
        json[hacker].hackathons[hackathon].prize
      );
      historystring += `</a>`;
